...
// you have this repeated many times here.
// image if you wanted to update. Do you want to do it in
// one place or four?
const remarkFields = 'text threadId parentRemId userId username';
...
//find by drink type
  ...
  .get('/drinkType/:drinkType', getPayload, (req, res, next) => {
    ...
    Thread.find({drinkType}).lean()
      // this is a case where it might have been better 
      // to inline the individual promises so that
      // you don't need to manually match them back: 
      .then(threads => {
        return threads.map((thread) => {
          return Remark.find({threadId: thread._id})
            .select(remarkFields).lean()
            .then((remark) => {
              // not sure what this object copying is about...
              let copy = JSON.parse(JSON.stringify(thread));
              copy.remark = remark;
              checkForOwner(copy, myPayload);
              return copy;
            });  
        });
      })
      .then(getThreads => Promise.all(getThreads))
